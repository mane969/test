...
=======
const CheckoutPage = ({ cartItems = [], setCartItems }) => { // <-- Added setCartItems
...
=======
    // --- THIS IS THE UPDATED handlePayment FUNCTION ---
    const handlePayment = async () => {
        setIsProcessing(true);

        // 1. Prepare the order data object
        const orderData = {
        ...
        try {
            // 2. Send the data to your server's /api/orders endpoint
            ...
            // 3. If successful, show the success screen
            setIsSuccess(true);
    ...
    // --- No changes to the rest of the file below this line ---
                ...
                <div className={`step-content ${activeStep === 1 ? 'visible' : ''}`}>
                    ...
                    <div className="form-row">
                        <div className="form-group">
                            <input type="text" name="firstName" value={formData.firstName} onChange={handleInputChange} className={formErrors.firstName ? 'error' : ''} placeholder=" " />
                            <label>First Name</label>
                            {formErrors.firstName && <p className="error-message">{formErrors.firstName}</p>}
                        </div>
                        <div className="form-group">
                            <input type="text" name="lastName" value={formData.lastName} onChange={handleInputChange} className={formErrors.lastName ? 'error' : ''} placeholder=" " />
                            <label>Last Name</label>
                            {formErrors.lastName && <p className="error-message">{formErrors.lastName}</p>}
                        </div>
                    </div>
                    <div className="form-group">
                        <input type="email" name="email" value={formData.email} onChange={handleInputChange} className={formErrors.email ? 'error' : ''} placeholder=" " />
                        <label>Email Address</label>
                        {formErrors.email && <p className="error-message">{formErrors.email}</p>}
                    </div>
                    <div className="form-group">
                        <input type="text" name="address" value={formData.address} onChange={handleInputChange} className={formErrors.address ? 'error' : ''} placeholder=" " />
                        <label>Full Address</label>
                        {formErrors.address && <p className="error-message">{formErrors.address}</p>}
                    </div>
                    <div className="form-row">
                        <div className="form-group">
                            <input type="text" name="city" value={formData.city} onChange={handleInputChange} className={formErrors.city ? 'error' : ''} placeholder=" " />
                            <label>City</label>
                            {formErrors.city && <p className="error-message">{formErrors.city}</p>}
                        </div>
                        <div className="form-group">
                            <input type="text" name="pincode" value={formData.pincode} onChange={handleInputChange} className={formErrors.pincode ? 'error' : ''} placeholder=" " />
                            <label>Pincode</label>
                     ...
                     <div className="delivery-option-group">
                        <label className="delivery-option">
                            <input
                                ...
                                checked={selectedDeliveryFee === standardDeliveryFee}
                                onChange={() => setSelectedDeliveryFee(standardDeliveryFee)}
                            />
                            <div className="option-content">
                                <span className="option-title">Standard Delivery</span>
                                <span className="option-details">3-4 hours</span>
                            </div>
                            <span className="option-price">
                                {standardDeliveryFee === 0 ? 'FREE' : `₹${standardDeliveryFee.toFixed(2)}`}
                            </span>
                        </label>
                        <label className="delivery-option">
                             <input
                                ...
                                checked={selectedDeliveryFee === expressDeliveryFee}
                                onChange={() => setSelectedDeliveryFee(expressDeliveryFee)}
                            />
                            <div className="option-content">
                                <span className="option-title">Express Delivery</span>
                                <span className="option-details">60-90 minutes</span>
                            </div>
                            <span className="option-price">₹{expressDeliveryFee.toFixed(2)}</span>
                        </label>
                     </div>
                ...
                <div className={`step-content ${activeStep === 3 ? 'visible' : ''}`}>
                    ...
                    <div className="payment-option-group">
                        ...
                        <label className="payment-option">
                            ...
                            <div className="payment-icon">
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"></circle><path d="M15 9.5a2.5 2.5 0 0 0-5 0 2.5 2.5 0 0 1-5 0"></path><path d="M12 12h.01"></path></svg>
                            </div>
                    ...
                    {paymentMethod === 'card' && (
                        <div className="payment-details-content">
                            <div className="form-group"><input type="text" placeholder=" " required /><label>Card Number</label></div>
                            <div className="form-group"><input type="text" placeholder=" " required /><label>Name on Card</label></div>
                            <div className="form-row">
                                <div className="form-group"><input type="text" placeholder=" " required /><label>Expiry (MM/YY)</label></div>
                                <div className="form-group"><input type="text" placeholder=" " required /><label>CVV</label></div>
                            </div>
                        </div>
                    )}

                    {paymentMethod === 'upi' && (
                        <div className="payment-details-content upi-content">
                        <div className="upi-qr-wrapper">
                            <img src="https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=upi://pay?pa=orders@delicia&pn=DeliciaBakery" alt="UPI QR Code" />
                            <div className="scanner-line"></div>
                        </div>
                        <p>Scan the code with any UPI app to pay <strong>₹{grandTotal.toLocaleString('en-IN')}</strong>.</p>
                    ...
                    {paymentMethod === 'cod' && (
                        <div className="payment-details-content cod-content">
                            <p>You can pay in cash when your order is delivered. Please try to have the exact amount ready for the delivery executive.</p>
                        </div>
                    )}
...
=======
                <div className="checkout-stepper">
                    <div className={`step ${activeStep >= 1 ? 'active' : ''} ${activeStep > 1 ? 'completed' : ''}`} onClick={() => !isProcessing && setActiveStep(1)}><div className="step-icon"><span>1</span><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg></div>Contact Info</div>
                    <div className="step-connector"></div>
                    <div className={`step ${activeStep >= 2 ? 'active' : ''} ${activeStep > 2 ? 'completed' : ''}`} onClick={() => !isProcessing && completedSteps[1] && setActiveStep(2)}><div className="step-icon"><span>2</span><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg></div>Delivery</div>
                    <div className="step-connector"></div>
                    <div className={`step ${activeStep >= 3 ? 'active' : ''} ${activeStep > 3 ? 'completed' : ''}`} onClick={() => !isProcessing && completedSteps[2] && setActiveStep(3)}><div className="step-icon"><span>3</span><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg></div>Payment</div>
                </div>
                <div className={`step-content ${activeStep === 1 ? 'visible' : ''}`}>
                    <h2 className="step-title">Shipping Address</h2>
                    <div className="form-row"><div className="form-group"><input type="text" name="firstName" value={formData.firstName} onChange={handleInputChange} className={formErrors.firstName ? 'error' : ''} placeholder=" " /><label>First Name</label>{formErrors.firstName && <p className="error-message">{formErrors.firstName}</p>}</div><div className="form-group"><input type="text" name="lastName" value={formData.lastName} onChange={handleInputChange} className={formErrors.lastName ? 'error' : ''} placeholder=" " /><label>Last Name</label>{formErrors.lastName && <p className="error-message">{formErrors.lastName}</p>}</div></div>
                    ...
                    <div className="form-row"><div className="form-group"><input type="text" name="city" value={formData.city} onChange={handleInputChange} className={formErrors.city ? 'error' : ''} placeholder=" " /><label>City</label>{formErrors.city && <p className="error-message">{formErrors.city}</p>}</div><div className="form-group"><input type="text" name="pincode" value={formData.pincode} onChange={handleInputChange} className={formErrors.pincode ? 'error' : ''} placeholder=" " /><label>Pincode</label>{formErrors.pincode && <p className="error-message">{formErrors.pincode}</p>}</div></div>
                    <button className="btn-next" onClick={() => handleNextStep(2)}>Next: Delivery</button>
                </div>
                <div className={`step-content ${activeStep === 2 ? 'visible' : ''}`}>
                    <h2 className="step-title">Delivery Method</h2>
                    <div className="delivery-option-group"><label className="delivery-option"><input type="radio" name="delivery" value={standardDeliveryFee} checked={selectedDeliveryFee === standardDeliveryFee} onChange={() => setSelectedDeliveryFee(standardDeliveryFee)} /><div className="option-content"><span className="option-title">Standard Delivery</span><span className="option-details">3-4 hours</span></div><span className="option-price">{standardDeliveryFee === 0 ? 'FREE' : `₹${standardDeliveryFee.toFixed(2)}`}</span></label><label className="delivery-option"><input type="radio" name="delivery" value={expressDeliveryFee} checked={selectedDeliveryFee === expressDeliveryFee} onChange={() => setSelectedDeliveryFee(expressDeliveryFee)} /><div className="option-content"><span className="option-title">Express Delivery</span><span className="option-details">60-90 minutes</span></div><span className="option-price">₹{expressDeliveryFee.toFixed(2)}</span></label></div>
                    <button className="btn-next" onClick={() => handleNextStep(3)}>Next: Payment</button>
                </div>
                <div className={`step-content ${activeStep === 3 ? 'visible' : ''}`}>
                    <h2 className="step-title">Payment Method</h2>
                    <p className="payment-note">All transactions are secure and encrypted.</p>
                    <div className="payment-option-group"><label className="payment-option"><input type="radio" name="payment" value="card" checked={paymentMethod === 'card'} onChange={() => setPaymentMethod('card')} /><div className="payment-icon"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="1" y="4" width="22" height="16" rx="2" ry="2"></rect><line x1="1" y1="10" x2="23" y2="10"></line></svg></div><span>Credit / Debit Card</span></label><label className="payment-option"><input type="radio" name="payment" value="upi" checked={paymentMethod === 'upi'} onChange={() => setPaymentMethod('upi')} /><div className="payment-icon"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"></circle><path d="M15 9.5a2.5 2.5 0 0 0-5 0 2.5 2.5 0 0 1-5 0"></path><path d="M12 12h.01"></path></svg></div><span>UPI (Google Pay, PhonePe)</span></label><label className="payment-option"><input type="radio" name="payment" value="cod" checked={paymentMethod === 'cod'} onChange={() => setPaymentMethod('cod')} /><div className="payment-icon"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg></div><span>Cash on Delivery</span></label></div>
